...
def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    ...
    def norm_cdf(x):
        # 计算标准正态分布的累积分布函数
        return (1. + torch.erf(torch.tensor(float(x) / np.sqrt(2.0)))) / 2.
...
# 简化的Conv2d，替换复杂的layers.py - 兼容预训练模型
class Conv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        if kernel_size > 1:
            ...
            self.ci = nn.Linear(self.g, out_channels // (in_channels // self.g) if in_channels // self.g > 0 else out_channels, bias=False)
...
class MODEL(nn.Module):
    def __init__(self, img_size=120, patch_size=4, embed_dim=96, num_heads=8, window_size=1, 
                 in_channel=2, out_channel=16, output_channel=1, mlp_ratio=4., qkv_bias=True, 
                 qk_scale=None, drop=0., attn_drop=0., patch_norm=True, depth=3, downsample=None,
                 drop_path=0., alpha=300, depth_p=32, norm_layer=nn.LayerNorm, use_checkpoint=False, 
                 bottleneck=True):
...
def fusion():
    ...
    model_path = r"C:\Users\Administrator\Downloads\MATR-main\MATR-main\models\model_10.pth"
    ...
    for num in range(1):
        ...
        path1 = r"C:\Users\Administrator\Downloads\MATR-main\extracted_frames1\frame_000034.bmp"
        path2 = r"C:\Users\Administrator\Downloads\MATR-main\MATR-main\fusion result\extracted_frames\frame_000034.bmp"
